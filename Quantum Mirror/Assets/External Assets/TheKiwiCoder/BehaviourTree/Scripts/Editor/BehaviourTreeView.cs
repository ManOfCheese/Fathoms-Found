    ...
    public class BehaviourTreeView : GraphView {
        ...
        public ScriptTemplate[] scriptFileAssets = {
            new ScriptTemplate{ templateFile = BehaviourTreeSettings.GetOrCreateSettings().scriptTemplateActionNode, 
                defaultFileName = "NewBTActionNode.cs", subFolder="Actions" },           
            new ScriptTemplate{ templateFile = BehaviourTreeSettings.GetOrCreateSettings().scriptTemplateBlackboardActionNode, 
                defaultFileName = "NewBTBlackbaordActionNode.cs", subFolder="BlackboardActions" },
            new ScriptTemplate{ templateFile = BehaviourTreeSettings.GetOrCreateSettings().scriptTemplateCompositeNode, 
                defaultFileName = "NewBTCompositeNode.cs", subFolder="Composites" },
            new ScriptTemplate{ templateFile = BehaviourTreeSettings.GetOrCreateSettings().scriptTemplateDecoratorNode, 
                defaultFileName = "NewBTDecoratorNode.cs", subFolder="Decorators" },
        };
        ...
        public NodeView FindNodeView( BTNode node ) {
        ...
        public override void BuildContextualMenu( ContextualMenuPopulateEvent evt ) {
            ...
            evt.menu.AppendAction( $"Create Script.../New Blackboard Node", ( a ) => CreateNewScript( scriptFileAssets[ 1 ] ) );
            ...
            {
                var types = TypeCache.GetTypesDerivedFrom<BTBlackBoardActionNode>();
                foreach ( var type in types )
                {
                    evt.menu.AppendAction( $"[Blackboard]/{type.Name}", ( a ) => CreateNode( type, nodePosition ) );
                }
            }
